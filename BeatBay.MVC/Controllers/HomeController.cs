...
namespace BeatBayMVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        // **M�todo privado para verificar autorizaci�n de Admin**
        ...
        // **M�todo privado para verificar autorizaci�n de Artist**
        ...
        // **M�todo privado para redirigir si no est� autorizado como Admin**
        private async Task<IActionResult> CheckAdminAuthorizationAndRedirectAsync()
        {
            if (!await IsUserAdminAsync())
            {
                TempData["Error"] = "No tienes permisos para acceder al panel de administraci�n";
                return RedirectToAction("Index", "Home");
            }
            return null;
        }

        // **M�todo privado para redirigir si no est� autorizado como Artist**
        ...
        public async Task<IActionResult> Index(string searchTerm)
        {
            ...
            if (currentUser != null)
            {
                ...
                // Si hay un término de búsqueda y el usuario está autenticado
        ...
        {
            // Verificar autorizaci�n antes de mostrar el panel de artista
            var authCheck = await CheckArtistAuthorizationAndRedirectAsync();
            if (authCheck != null) return authCheck;

            // Si llega aqu�, el usuario es artista
            var userDataJson = HttpContext.Session.GetString("UserData");
            UserDto currentUser = null;
            if (!string.IsNullOrEmpty(userDataJson))
            {
        ...
        {
            // Verificar autorizaci�n antes de mostrar el panel
            var authCheck = await CheckAdminAuthorizationAndRedirectAsync();
            if (authCheck != null) return authCheck;

            // Si llega aqu�, el usuario es admin
            var userDataJson = HttpContext.Session.GetString("UserData");
            UserDto currentUser = null;
            if (!string.IsNullOrEmpty(userDataJson))
            {
